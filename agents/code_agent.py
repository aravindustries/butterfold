...
def _golden_hint(name: str) -> str:
    ...
    if name == "scheduler_addr_control":
        return (
            ...
            "  stage 0..6 (outer), group grp (middle), kk (inner).\n"
            "  CRITICAL: half and m are COMBINATIONAL, derived from the current stage —\n"
            "  do NOT store them in registers (a registered `half` updated as\n"
            "  `half <= 1<<stage` uses the OLD stage and desyncs). Use continuous assigns:\n"
            "    wire [6:0] half = 7'd1 << stage;   wire [7:0] m = 8'd1 << (stage + 1);\n"
            "  Only stage, grp, kk, cnt, active are registers.\n"
