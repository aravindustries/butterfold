...
A = B     = 9                     # twiddle ROM precision (bits). EVM is output-
                                  # quantization limited, so 9 bits matches 12/15
                                  # (seed-42 EVM 1.59%) while halving datapath width.
SHIFT     = A + B + 7             # final round-shift: 1/(2^A * 2^B) * 1/M(=2^7)

# ── Datapath bit-widths (sized to worst case, minimum-area) ─────────────────
# Twiddle: |w| <= 2^A = 512  -> 11-bit signed; use 12 for margin.
# spread : |sum_12 byte(127) * w(512)| <= 779_264  -> 21-bit signed; use 24.
# acc    : |sum_12 spread * w128| <= ~4.8e9         -> 34-bit signed; use 40.
TW_W     = 12                     # twiddle ROM word width
SPREAD_W = 24                     # spread bin width
ACC_W    = 40                     # accumulator / product width
...
# ── Verilog emission ─────────────────────────────────────────────────────────
def _rom_case(name, values, idx_bits, width=TW_W):
...
def _idx12_lut() -> str:
    """LUT for (n*j) mod 12, replacing a runtime modulo-12 divider on the
    critical path. Keyed on {n[3:0], j[3:0]} for n,j in 0..11."""
    lines = ["  function [3:0] idx12;",
             "    input [3:0] nn;",
             "    input [3:0] jj;",
             "    case ({nn, jj})"]
    for n in range(12):
        for j in range(12):
            key = (n << 4) | j
            lines.append(f"      8'h{key:02x}: idx12 = 4'd{(n*j) % 12};")
    lines.append("      default: idx12 = 4'd0;")
    lines.append("    endcase")
    lines.append("  endfunction")
    return "\n".join(lines)
...
module butterfold_top (
  ...
  (* mem2reg *) reg signed [{SPREAD_W-1}:0] spread_re [0:11];
  (* mem2reg *) reg signed [{SPREAD_W-1}:0] spread_im [0:11];

  // ── Shared accumulator and pending output bytes ─────────────────────────
  reg signed [{ACC_W-1}:0] acc_re, acc_im;
...
{rom_idx12}
  ...
  reg signed [{SPREAD_W-1}:0] mult_ar, mult_ai;   // holds inbuf(8b) and spread
  reg signed [{TW_W-1}:0]     mult_br, mult_bi;   // twiddle word
  ...
  wire signed [{ACC_W-1}:0] mult_pr = mult_ar * mult_br - mult_ai * mult_bi;
  wire signed [{ACC_W-1}:0] mult_pi = mult_ar * mult_bi + mult_ai * mult_br;

  // Full sums for the final term of an accumulation
  wire signed [{ACC_W-1}:0] fin_re = acc_re + mult_pr;
  wire signed [{ACC_W-1}:0] fin_im = acc_im + mult_pi;

  // Round-shift + saturate to int8 (for the final IFFT term)
  wire signed [{ACC_W-1}:0] shr_re = (fin_re + ({ACC_W}'sd1 <<< ({SHIFT} - 1))) >>> {SHIFT};
  wire signed [{ACC_W-1}:0] shr_im = (fin_im + ({ACC_W}'sd1 <<< ({SHIFT} - 1))) >>> {SHIFT};
  wire signed [7:0]  fbyte_re =
      (shr_re >  {ACC_W}'sd127) ?  8'sd127 : (shr_re < -{ACC_W}'sd128) ? -8'sd128 : shr_re[7:0];
  wire signed [7:0]  fbyte_im =
      (shr_im >  {ACC_W}'sd127) ?  8'sd127 : (shr_im < -{ACC_W}'sd128) ? -8'sd128 : shr_im[7:0];
      ...
      case (state)
        ...
        S_DFT: begin
          ...
          if (dft_n == 4'd11) begin
            spread_re[dft_j] <= fin_re[{SPREAD_W-1}:0];   // last term + store
            spread_im[dft_j] <= fin_im[{SPREAD_W-1}:0];
            acc_re <= {ACC_W}'sd0;
            acc_im <= {ACC_W}'sd0;
            dft_n  <= 4'd0;
...
# ── Hybrid emission: locked kernel + agent-refinable hierarchical top ────────
# The kernel holds every bit-exact-fragile element (twiddle ROMs, the shared
# complex multiplier, and the round-shift+saturate). The wrapper/top holds the
# control that LLM agents can safely author: the FSM, the input deserializer,
# the CP + centered-map address generator, and the output serializer. Splitting
# here lets the agentic code step genuinely write the wrapper while the DSP
# kernel stays generated-and-validated.
KERNEL_OUT = ROOT / "butterfold_kernel.v"
TOP_OUT    = ROOT / "butterfold_top.v"


def emit_kernel() -> str:
    """LOCKED bit-exact datapath: twiddle ROMs + complex multiply + round/clip."""
    rom_w12_re  = _rom_case("w12_re",  W12_RE,  4)
    rom_w12_im  = _rom_case("w12_im",  W12_IM,  4)
    rom_w128_re = _rom_case("w128_re", W128_RE, 7)
    rom_w128_im = _rom_case("w128_im", W128_IM, 7)
...
//
// Frozen params: K={K}, M={M}, CP={CP}, START={START}, A=B={A}, SHIFT={SHIFT}

`timescale 1ns/1ps
module butterfold_kernel (
  input  wire signed [{SPREAD_W-1}:0] a_re,    // operand A real (inbuf byte or spread bin)
  input  wire signed [{SPREAD_W-1}:0] a_im,    // operand A imag
  ...
  output wire signed [{ACC_W-1}:0] prod_re,    // complex product a * W (real)
  output wire signed [{ACC_W-1}:0] prod_im,    // complex product a * W (imag)
  input  wire signed [{ACC_W-1}:0] sum_re,     // full accumulated sum (real) to round
  input  wire signed [{ACC_W-1}:0] sum_im,     // full accumulated sum (imag) to round
...
{rom_idx12}
  ...
  wire signed [{TW_W-1}:0] b_re = use_ifft ? w128_re(w_idx) : w12_re(d12);
  wire signed [{TW_W-1}:0] b_im = use_ifft ? w128_im(w_idx) : w12_im(d12);
  ...
  wire signed [{ACC_W-1}:0] shr_re = (sum_re + ({ACC_W}'sd1 <<< ({SHIFT} - 1))) >>> {SHIFT};
  wire signed [{ACC_W-1}:0] shr_im = (sum_im + ({ACC_W}'sd1 <<< ({SHIFT} - 1))) >>> {SHIFT};
  assign byte_re =
      (shr_re >  {ACC_W}'sd127) ?  8'sd127 : (shr_re < -{ACC_W}'sd128) ? -8'sd128 : shr_re[7:0];
  assign byte_im =
      (shr_im >  {ACC_W}'sd127) ?  8'sd127 : (shr_im < -{ACC_W}'sd128) ? -8'sd128 : shr_im[7:0];
...
def emit_top() -> str:
    """Hierarchical reference top: FSM + CP addressing + accumulators around u_kernel.

    This is the AGENT-REFINABLE wrapper. It is functionally identical to the flat
    butterfold_reference.v but factors the bit-exact datapath out into
    butterfold_kernel. It serves both as the guaranteed-correct fallback and as the
    reference the code agent refines into per-subtask submodules.
    """
    return f"""\
...
module butterfold_top (
  ...
  // tau implements cyclic-prefix ordering: the last CP={CP} IFFT samples are
  // emitted first (samp < CP), then the body samples 0..M-1.
  reg signed [{SPREAD_W-1}:0] a_re, a_im;   // operand A into the kernel
  reg        use_ifft;
  reg  [6:0] tau;                           // IFFT time index (CP-adjusted)
  reg  [6:0] w_idx;                         // ((START+j)*tau) mod 128
  ...
  wire signed [{ACC_W-1}:0] prod_re, prod_im;
  wire signed [7:0] byte_re, byte_im;
  wire signed [{ACC_W-1}:0] fin_re = acc_re + prod_re;   // full sum incl. final term
  wire signed [{ACC_W-1}:0] fin_im = acc_im + prod_im;
      ...
      case (state)
        ...
        S_EMIT: begin
          ...
          if (emit_half == 1'b0) begin
            ...
            dout      <= out_q_byte;
            emit_half <= 1'b0;
            if (samp == {M+CP-1}) begin         // {M+CP-1} = last sample
              state <= S_IDLE;
              done  <= 1'b1;
            end else begin
              samp   <= samp + 8'd1;
...
def main():
    ...
    # Flat single-module reference (unchanged — ultimate fallback / golden).
    OUT.write_text(emit_verilog(), encoding="utf-8")
    print(f"[gen] Wrote {OUT.name}  ({OUT.read_text().count(chr(10))} lines)")

    # Hybrid split: locked kernel + agent-refinable hierarchical top.
    KERNEL_OUT.write_text(emit_kernel(), encoding="utf-8")
    TOP_OUT.write_text(emit_top(), encoding="utf-8")
    print(f"[gen] Wrote {KERNEL_OUT.name}  ({KERNEL_OUT.read_text().count(chr(10))} lines)  [LOCKED]")
    print(f"[gen] Wrote {TOP_OUT.name}  ({TOP_OUT.read_text().count(chr(10))} lines)  [hierarchical]")
