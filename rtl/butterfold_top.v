...
module butterfold_top (
    ...
    // ------------------------------------------------------------------
    // Observability tie-off. The authored FDIQ/TDIQ adapters do not drive
    // their external output bytes, so `dout` alone is structurally constant
    // and synthesis would prune the whole datapath (memory, multiplier,
    // buffers). To get a faithful PPA of the authored logic we fold every
    // otherwise-unobserved net — status flags AND the wide internal data
    // buses — into a sticky aggregate exposed on the scan pin. This keeps the
    // core scratch memory + shared multiplier + module buffers in the netlist.
    // ------------------------------------------------------------------
    wire status_bits = core_uop_done ^ core_overflow ^ core_sat
                     ^ (^core_read_data) ^ core_read_valid
                     ^ sch_error ^ sch_busy ^ sch_input_bank ^ sch_output_bank
                     ^ sch_cmd_ready
                     ^ map_config_error ^ map_busy ^ map_in_ready
                     ^ (^map_out_data) ^ map_out_valid ^ map_out_last
                     ^ (^map_mem_addr) ^ map_mem_write ^ (^map_mem_wdata)
                     ^ fdiq_busy ^ fdiq_done ^ fdiq_iq_err ^ fdiq_fd_in_last
                     ^ (^fdiq_fd_in_data) ^ fdiq_fd_in_valid
                     ^ tdiq_busy ^ tdiq_cp_error ^ tdiq_sc_error ^ tdiq_iq_err
                     ^ tdiq_rx_symbol_last ^ (^tdiq_rx_symbol_data)
                     ^ tdiq_rx_symbol_valid ^ (^tdiq_tx_rd_addr) ^ tdiq_tx_rd_req;
    reg status_sticky;
