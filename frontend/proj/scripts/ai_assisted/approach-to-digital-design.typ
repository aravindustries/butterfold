#set document(title: "Approach to Digital Design", author: "Project Team")
#set page(numbering: "1")
#set text(lang: "en")

#title[Approach to Digital Design with SystemVerilog]

= General Approach

== Setting the Stage
- Write extensive textual descriptions of the expected functionality of the block, including opportunities for improvements, test plan, and questions/concerns. This becomes the spec sheet of that particular block and will be appended to the primary project document.
- Develop the port list needed for proper functioning.
  - Decide on the kind of interface to the outside world, such as using plain ports, interface/struct bundle, or protocols like AXI or UART.
- Develop timing diagrams (in `wavedrom`) to demonstrate the response of the design to input changes.
- Prepare a top-down hierarchical `data_path` level implementation block diagram (on **draw.io**) down to register & comb logic level.
  - Decide on compulsory pipeline stages (e.g., fixed memory latency).
  - Identify spec compliance mandatory clock domain crossings. Group different clock domains using the 4 color-theorem.
- Develop functional assertions & coverage conditions (in code and/or text).
  - Refer to the assertions page to see which kinds of assertions apply where.

== Testbenching
- Build an algorithmic twin of the expected implementation in a high-level language (C++/Python) for proof of concept & use as a golden model during verification.
  - Supply high-level test cases to the golden_ref & the design (using DPI) simultaneously & compare responses.
  - After synthesis, the processor can be loaded with those test cases to exercise the PL.

== Design Coding
- Begin with the control path & prove spec compliance with simulation output timing diagrams. Code the data path at the end. Write assertions where possible.
- Perform rigorous unit tests using the test plan to check for:
  - Functional bugs
  - Wasted cycles

== Pipelining
=== Architectural/Logical Pipeline Stages
- Stages defined by the architect to distinguish between operations, generally implemented in different modules.
- Need not correlate with the actual number of registers.
- Can define further hierarchies of architectural & implementational pipeline stages.
- Usually operate at different frequencies due to task demands.
  - Can communicate with other stages over interfaces or structs.
  - Can use FIFO structures or bus protocols (AXI, AHB, Wishbone, etc).

=== Implementational/Physical Pipeline Stages
- Flopped stages inside an architectural stage to break up the critical path.
- Must operate at the same clock frequency and do not use handshake protocols (may use synchronous FIFOs for infrequent data).
- Only the primary data path(s) is flopped manually with `always_ff`; other signals are pipelined with `struct`.
...
- in assertions: until (until_with), throughout, implies, intersect, within, first_match, .ended, .matched

= Directory Structure Reference
> This is not updated frequently & cannot be identical to any specific project directory structure. For the role of a directory, refer to the `README.md` in that folder.
...
= Easter Eggs
- As a fun thought experiment, consider how to put Easter eggs into code, like how tayagaraja inserted his name into his songs, or how chips and PCBs have special messages that cannot be removed easily. This can be a form of copyright protection.

= To Do
- [ ] Calculating the depth of a FIFO using deduction and empirically. For example, if the clock speed up is 2x from left to right domain, the depth needs to be 2.
- [ ] How does 2 FF crossing work? What are its limitations?
- [ ] How different is it from reset domain crossing?
- [ ] Unify coding methodologies & guidelines from Notion to Obsidian.
- [ ] How to use the worst negative slack timing report to identify the critical path and find the maximum frequency of operation. (TCL)
...
- [ ] Learn the art of converting simple testbenches with initial & wait statements into synthesizable code for demonstration with ILA.
- [ ] Learn to write code that is friendly to synthesizable testbench code & to the previous blocks.
