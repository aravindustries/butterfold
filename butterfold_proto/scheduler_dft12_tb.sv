...
module scheduler_dft12_tb;
    ...
    initial begin
        ...
        $display("============================================================");
        $display("DFT12 regression");
        $display("============================================================");

        for (test_number = 0;
             test_number < NUM_DFT12_TESTS;
             test_number = test_number + 1) begin
        ...
        $display("");
        $display("DFT12 tests:            %0d/%0d passed",
                 dft12_passed, NUM_DFT12_TESTS);
        $display("DFT12 components:       %0d/%0d exact (%0.2f%%)",
                 dft12_exact_components, dft12_total_components,
                 100.0 * dft12_exact_components / dft12_total_components);
        $display("DFT12 mean error:       %0.6f LSB; max=%0d LSB",
                 1.0 * dft12_absolute_error_sum / dft12_total_components,
                 dft12_maximum_error);
        $display("DFT12 avg command->last: %0.2f cycles",
                 1.0 * dft12_command_to_last_sum / NUM_DFT12_TESTS);
        $display("DFT12 avg input->last:   %0.2f cycles",
                 1.0 * dft12_input_to_last_sum / NUM_DFT12_TESTS);
        $display("");
        ...
        total_failed =
            two_point_failed + fft3_failed + dft12_failed +
    ...
    initial begin
        $dumpfile("scheduler_dft12_tb.vcd");
        $dumpvars(0, scheduler_dft12_tb);
    end
