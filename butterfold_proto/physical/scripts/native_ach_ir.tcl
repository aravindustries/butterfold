# Fresh OpenRCX + PDNSim IR on the native-ring + ACH-shell filled ODB.
...
# Reuse the same 6-north VDD / 6-west VSS sources as the native-ring IR.
if {![file exists $out/irdrop/VDD.vsrc]} {
  file copy -force $native/irdrop/VDD.vsrc $out/irdrop/VDD.vsrc
}
if {![file exists $out/irdrop/VSS.vsrc]} {
  file copy -force $native/irdrop/VSS.vsrc $out/irdrop/VSS.vsrc
}
puts "NATIVE_ACH_IR_VDD"
analyze_power_grid -net VDD -vsrc $out/irdrop/VDD.vsrc -voltage_file $out/irdrop/native-ach-VDD.csv
puts "NATIVE_ACH_IR_VSS"
analyze_power_grid -net VSS -vsrc $out/irdrop/VSS.vsrc -voltage_file $out/irdrop/native-ach-VSS.csv
report_power -digits 6 > $out/vectorless_power.rpt
puts "NATIVE_ACH_IR_POWER_DONE"
exit
