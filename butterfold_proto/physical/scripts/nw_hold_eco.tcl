# Hold ECO on the North/West extracted-closed setup ODB. FF liberty + min RCX.
...
if {[info exists env(ECO_SRC)] && $env(ECO_SRC) ne ""} {
  ...
  set src $eco/butterfold_top_eco_routed.odb
}
...
report_wns -min
report_tns -min
report_wns -min > $out/before_wns.rpt
report_tns -min > $out/before_tns.rpt
...
report_wns -min
report_tns -min
report_wns -min > $out/after_wns.rpt
report_tns -min > $out/after_tns.rpt
