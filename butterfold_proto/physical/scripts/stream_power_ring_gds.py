#!/usr/bin/env python3
"""KLayout DEF→GDS streamout for the power-ring ECO (same method as signed-off ACH)."""
...
DEF_PATH = Path(
    os.environ.get("ROUTE_DEF", PROTO / "physical/results/m2_fix/power_ring.def")
)
GDS_PATH = Path(
    os.environ.get(
        "OUTPUT_GDS",
        PROTO / "physical/results/m2_fix/candidate_power_ring_klayout/butterfold_top.gds",
    )
)
...
for p in (DEF_PATH, TECH_LEF, CELL_LEF, SRAM_LEF, SC_GDS, SRAM_GDS, LYM):
    if not p.is_file():
...
print("[INFO] top instances", top.child_instances(), "cells", layout.cells())
...
print(f"GDS_DBU {dbu}")
print(f"GDS_BBOX_UM {bbox.width()*dbu:.4f} {bbox.height()*dbu:.4f}")
