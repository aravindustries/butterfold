# Native extract + STA (no SPEF round-trip). CORNER=ss_setup|ff_hold
...
if {[info exists env(STA_ODB)] && $env(STA_ODB) ne ""} {
  set odb $env(STA_ODB)
} elseif {[file exists $eco/hold_eco/routed.odb]} {
...
if {$mode eq "ff_hold"} {
  ...
  report_checks -from [get_ports dout_ready_i] -path_delay max -group_path_count 10 \
    > $out/dout_ready_extract.rpt
