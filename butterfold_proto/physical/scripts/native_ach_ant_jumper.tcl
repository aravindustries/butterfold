# Last remaining antenna is Metal2 side-area on SRAM Q[4] / mux2 I1.
# Diodes cannot close side-area; insert jumpers then incremental DRT.
set proto /headless/aravindustries-repos/butterfold/butterfold_proto
set out $proto/physical/results/native_power_ring_ach
set src $out/ach_ant_routed_ant1.odb
...
pg_connect
set block [ord::get_db_block]
set pin_inst [$block findInst _17993_]
if {$pin_inst eq "NULL" || $pin_inst eq ""} {
  puts "MISSING_INST _17993_"
} else {
  set bbox [$pin_inst getBBox]
  puts "INST _17993_ [[$pin_inst getMaster] getName] [$bbox xMin] [$bbox yMin] [$bbox xMax] [$bbox yMax]"
  set iterm [$pin_inst findITerm I1]
  set net [$iterm getNet]
  puts "NET [$net getName] ITERMS [llength [$net getITerms]] BTERMS [llength [$net getBTerms]]"
}
set_thread_count 16
set_routing_layers -signal Metal2-Metal5 -clock Metal2-Metal5
foreach layer {Metal2 Metal3 Metal4 Metal5} { set_global_routing_layer_adjustment $layer 0.3 }
set_global_routing_layer_adjustment Metal2 0.55
puts "ANT_BEFORE [check_antennas] DIODE [diode_count]"
# Jumpers need GRT guides; DRT-only ODBs have none, so rebuild guides first.
ensure_pad_spacing_obs
puts "GRT_FOR_JUMPERS"
global_route -congestion_iterations 50 -verbose -allow_congestion
set j [repair_antennas gf180mcu_fd_sc_mcu9t5v0__antenna -jumper_only -allow_congestion]
puts "REPAIR_JUMPER $j DIODE [diode_count]"
if {[catch {check_placement -verbose} m]} { puts "PLACE $m"; exit 1 }
pg_connect
detailed_route -droute_end_iter 64 -or_seed 42 -verbose 1 -output_drc $out/drt/ach_ant_jumper.drc
remove_pad_spacing_obs
set ant [check_antennas]
puts "ANT_AFTER $ant DIODE [diode_count]"
if {$ant} {
  check_antennas -verbose -report_file $out/antenna/post_jumper.rpt
}
