# Final native-ring STA. CORNER=max_ss|min_ff  (default max_ss)
...
if {$corner eq "max_ss"} {
  ...
  if {[catch {puts "RESET_VISIBLE_SLEW [sta::max_slew_violation_count] RESET_VISIBLE_CAP [sta::max_capacitance_violation_count]"} m]} {
    puts "RESET_VISIBLE_CAUGHT $m"
  }
  report_check_types -max_slew -max_capacitance -max_fanout -violators -digits 4 > $out/electrical_reset_visible.rpt
} elseif {$corner eq "nom_tt"} {
  set c nom_tt_025C_5v00
  define_corners $c
  read_liberty -corner $c $pdk/libs.ref/gf180mcu_fd_sc_mcu9t5v0/lib/gf180mcu_fd_sc_mcu9t5v0__tt_025C_5v00.lib
  read_liberty -corner $c $pdk/libs.ref/gf180mcu_fd_ip_sram/lib/gf180mcu_fd_ip_sram__sram256x8m8wm1__tt_025C_5v00.lib
  read_db $odb
  read_sdc $proto/physical/constraints.sdc
  catch {set_output_delay 0.0 -clock core_clk [get_ports {din_ready_o_OUT dout_valid_o_OUT dout_OUT[*]}]}
  set_propagated_clock [all_clocks]
  set spef [file dirname $odb]/spef_sta/final.nom.spef
  file mkdir [file dirname $spef]
  if {![file exists $spef]} {
    define_process_corner -ext_model_index 0 CURRENT_CORNER
    extract_parasitics -ext_model_file $pdk/libs.tech/librelane/rules.openrcx.gf180mcuD.nom -lef_res
    write_spef $spef
  }
  read_spef -corner $c $spef
  puts "SETUP_WNS"
  ...
  report_check_types -max_slew -max_capacitance -max_fanout -violators -digits 4 > [file dirname $odb]/electrical_nom_tt.rpt
  ...
  set_output_delay 0.0 -clock core_clk [get_ports {din_ready_o_OUT dout_valid_o_OUT dout_OUT[*]}]
  set_propagated_clock [all_clocks]
  set spef [file dirname $odb]/spef_sta/final.min.spef
  file mkdir [file dirname $spef]
  if {![file exists $spef]} {
    define_process_corner -ext_model_index 0 CURRENT_CORNER
