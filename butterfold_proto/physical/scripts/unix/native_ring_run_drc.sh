#!/usr/bin/env bash
# Authoritative GF180 separate-table non-fill DRC + MSLOT on the native-ring GDS.
set -euo pipefail
PROTO=/headless/aravindustries-repos/butterfold/butterfold_proto
GDS=${1:-$PROTO/physical/results/native_power_ring/candidate/butterfold_top.gds}
OUT=${2:-$PROTO/physical/results/native_power_ring/klayout_drc}
...
# MSLOT: split-table TABLE_NAME=mslot crashes in this PDK. Use the established
# unified deck with table_name=main (m2_fix / ACH methodology).
