# ECO6b: same 287 class swaps as ECO6, but rip CLOCK as well as
# data. ECO6 DRT-0206 was stale clknet_leaf_* after FF displacement.
...
foreach inst [$block getInsts] {
  ...
  if {[string match "*g_pin_drive*" $n] && [string match "*u_*_driver*" $n]} {
    ...
    if {[string match "*__buf_*" $mn] && $mn ne "gf180mcu_fd_sc_mcu9t5v0__buf_8" && $mn ne "gf180mcu_fd_sc_mcu9t5v0__buf_16" && $buf8 ne "NULL"} {
      if {[catch {$inst swapMaster $buf8}]} {} else { incr nsram }
    }
  }
}
puts "ECO6B_SRAM_DRV $nsram"
...
global_route -congestion_iterations 50 -verbose -guide_file $eco/eco6b.guide
detailed_route -droute_end_iter 64 -or_seed 42 -verbose 1 -output_drc $eco/eco6b.drc
write_db $eco/butterfold_top_eco6b_routed.odb
write_def $eco/butterfold_top_eco6b_routed.def

set rcx_max $pdk/libs.tech/librelane/rules.openrcx.gf180mcuD.max
if {[catch {
  extract_parasitics -ext_model_file $rcx_max
  write_spef $eco/eco6b.max.spef
  read_spef $eco/eco6b.max.spef
} emsg]} { puts "EXTRACT_WARN $emsg" }
puts "ECO6B_EXTRACTED_WNS"
report_wns -max
report_tns -max
catch {report_wns -max > $out/eco6b_spef_wns.rpt}
catch {report_tns -max > $out/eco6b_spef_tns.rpt}
catch {report_checks -path_delay max -slack_max 0 -group_path_count 40 > $out/eco6b_spef_violations.rpt}
catch {report_check_types -max_slew -max_cap -max_fanout -violators > $out/eco6b_spef_electrical.rpt}
puts "ECO6B_COMPLETE"
exit
