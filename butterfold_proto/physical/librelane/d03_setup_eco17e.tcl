# ECO17e: SRAM GWEN nor3_1/_gwen clkinv only. Preserve CLOCK. No repair_timing.
...
foreach net [$block getNets] {
  ...
  if {[string match "clknet_*" $nn] || [string match "eco14_skew*" $nn] || [string match "eco15_*" $nn] || [string match "eco16*" $nn]} { continue }
...
global_route -congestion_iterations 50 -verbose -guide_file $eco/eco17e.guide
detailed_route -droute_end_iter 64 -or_seed 42 -verbose 1 -output_drc $eco/eco17e.drc
write_db $eco/butterfold_top_eco17e_routed.odb
write_def $eco/butterfold_top_eco17e_routed.def

set rcx_max $pdk/libs.tech/librelane/rules.openrcx.gf180mcuD.max
if {[catch {
  extract_parasitics -ext_model_file $rcx_max
  write_spef $eco/eco17e.max.spef
  read_spef $eco/eco17e.max.spef
} emsg]} { puts "EXTRACT_WARN $emsg" }
puts "ECO17E_EXTRACTED_WNS"
report_wns -max
report_tns -max
catch {report_wns -max > $out/eco17e_spef_wns.rpt}
catch {report_tns -max > $out/eco17e_spef_tns.rpt}
catch {report_checks -path_delay max -slack_max 0 -group_path_count 20 > $out/eco17e_spef_violations.rpt}
catch {report_check_types -max_slew -max_cap -max_fanout -violators > $out/eco17e_spef_electrical.rpt}
puts "ECO17E_COMPLETE"
exit
