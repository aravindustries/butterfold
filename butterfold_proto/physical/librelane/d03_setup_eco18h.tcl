...
if {[catch {detailed_route -droute_end_iter 32 -or_seed 42 -verbose 1 -output_drc $eco/eco18h.drc} dmsg]} {
  puts "DRT_WARN $dmsg"
} else { puts "DRT_OK" }
set n2b [$block findNet eco18_net_18433]
puts "NEW_NET_WIRE [$n2b getWire]"
write_db $eco/butterfold_top_eco18h.odb
write_def $eco/butterfold_top_eco18h.def
set rcx_max $pdk/libs.tech/librelane/rules.openrcx.gf180mcuD.max
if {[catch {
  extract_parasitics -ext_model_file $rcx_max
  write_spef $eco/eco18h.max.spef
  read_spef $eco/eco18h.max.spef
} emsg]} { puts "EXTRACT_WARN $emsg" }
puts "ECO18H_EXTRACTED_WNS"
report_wns -max
report_tns -max
report_checks -path_delay max -group_path_count 5
if {[catch {check_placement -verbose} m2]} { puts "PLACE2 $m2" } else { puts "PLACE2_OK" }
catch {report_wns -max > $out/eco18h_spef_wns.rpt}
catch {report_tns -max > $out/eco18h_spef_tns.rpt}
catch {report_checks -path_delay max -slack_max 0 -group_path_count 10 > $out/eco18h_spef_violations.rpt}
puts "ECO18H_COMPLETE"
exit
