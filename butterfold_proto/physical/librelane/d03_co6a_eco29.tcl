# eco29: same R180 as eco28, but DRT only the 38 ripped nets via set_nets_to_route.
# eco28 full TritonRoute rewrote chip-wide via landings (KLayout M2.3 min-area).
...
foreach name $insts {
    ...
    foreach t [$i getITerms] {
        ...
        dict set dummy [$n getName] $n
    }
}
puts "RIP_COUNT [dict size $dummy]"
dict for {nn n} $dummy {
    lappend ripnames $nn
...
puts "SET_NETS_TO_ROUTE [llength $ripnames]"
set_nets_to_route $ripnames
foreach layer {Metal2 Metal3 Metal4 Metal5} { set_global_routing_layer_adjustment $layer 0.3 }
if {[catch {global_route -congestion_iterations 30 -verbose -guide_file $eco/co6a29.guide} gmsg]} {
    puts "GRT_WARN $gmsg"
}
if {[catch {detailed_route -droute_end_iter 32 -or_seed 42 -verbose 1 -output_drc $eco/co6a29.drc} dmsg]} {
    puts "DRT_WARN $dmsg"
} else { puts "DRT_OK" }

puts "UNROUTED"
if {[catch {design_is_routed -verbose} rmsg]} { puts "ROUTED_CHECK $rmsg" } else { puts "DESIGN_ROUTED_OK" }
...
write_spef $cand/co6a29.max.spef
read_spef $cand/co6a29.max.spef
...
write_db $cand/butterfold_top_co6a29.odb
write_def $cand/butterfold_top_co6a29.def
write_verilog -include_pwr_gnd $cand/butterfold_top.co6a29.pnl.v
puts "CO6A29_DONE"
exit
