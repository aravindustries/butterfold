# ECO17 from eco16b. Remaining: _18433_ -0.08 and SRAM GWEN nor3_1.
# Swap nor3_1; upsize GWEN inverters; one extra capture buf on _18433_.
# Do NOT destroy existing wires (incremental route).
...
if {$i ne "NULL" && $n3 ne "NULL"} {
  if {[catch {$i swapMaster $n3} msg]} { puts "SWAP_FAIL _09881_ $msg" } else { puts "SWAP _09881_ nor3_4" }
}
foreach name {
  ...
  if {$i ne "NULL" && $i ne "" && $inv8 ne "NULL"} {
    if {[catch {$i swapMaster $inv8} msg]} { puts "SWAP_FAIL $name $msg" } else { puts "SWAP $name clkinv_8" }
  }
}

# Extra capture delay on the last stdcell violator only.
...
if {$ff ne "NULL"} {
  ...
  puts "SKEW _18433_ extra clkbuf_16"
}
...
puts "ECO17_INCREMENTAL_ROUTE"
# Keep existing wires. GRT/DRT fill in new nets only.
foreach layer {Metal2 Metal3 Metal4 Metal5} {
  set_global_routing_layer_adjustment $layer 0.3
}
catch {global_connect}
if {[catch {global_route -congestion_iterations 20 -verbose -guide_file $eco/eco17.guide} gmsg]} {
  puts "GRT_WARN $gmsg"
}
detailed_route -droute_end_iter 64 -or_seed 42 -verbose 1 -output_drc $eco/eco17.drc
write_db $eco/butterfold_top_eco17_routed.odb
write_def $eco/butterfold_top_eco17_routed.def

set rcx_max $pdk/libs.tech/librelane/rules.openrcx.gf180mcuD.max
if {[catch {
  extract_parasitics -ext_model_file $rcx_max
  write_spef $eco/eco17.max.spef
  read_spef $eco/eco17.max.spef
} emsg]} { puts "EXTRACT_WARN $emsg" }
puts "ECO17_EXTRACTED_WNS"
report_wns -max
report_tns -max
catch {report_wns -max > $out/eco17_spef_wns.rpt}
catch {report_tns -max > $out/eco17_spef_tns.rpt}
catch {report_checks -path_delay max -slack_max 0 -group_path_count 20 > $out/eco17_spef_violations.rpt}
catch {report_check_types -max_slew -max_cap -max_fanout -violators > $out/eco17_spef_electrical.rpt}
puts "ECO17_COMPLETE"
exit
