#!/usr/bin/env bash
# Same-GDS KLayout on Mag pgfix SHA 969dff47. Proven method:
#   run_drc.py --mp=4 flat, then solo contact/ldpmos/nat if OOM,
#   density_only, antenna_only, unified MSLOT (table_name=main).
set -euo pipefail
...
DRC=$BASE/klayout_drc_pgfix
DEN=$BASE/klayout_density_pgfix
ANT=$BASE/klayout_antenna_pgfix
MSL=$BASE/klayout_mslot_pgfix
...
# layers_def.drc is sourced relatively from the deck; run from $MSL
(
  cd "$MSL"
