# Extracted-SPEF setup repair on eco36 setup ODB. Do not replace SPEF with placement RC.
set proto /headless/aravindustries-repos/butterfold/butterfold_proto
set pdk /foss/pdks/gf180mcuD
set src $proto/physical/results/d03_ach_candidate/co6a36/setup_eco/butterfold_top_co6a36_setup.odb
set spef $proto/physical/results/d03_ach_candidate/co6a36/setup_eco/after.max.spef
set outdir $proto/physical/results/d03_ach_candidate/co6a36/setup_eco2
file mkdir $outdir
puts "ECO_SRC $src"
...
foreach inst [$block getInsts] {
  if {[string match *sram256x8m8wm1* [[$inst getMaster] getName]]} { $inst setPlacementStatus FIRM }
}
...
puts "REPAIR_DESIGN"
if {[catch {repair_design -slew_margin 20 -cap_margin 20} m]} { puts "REPAIR_DESIGN_WARN $m" }
puts "REPAIR_TIMING"
if {[catch {repair_timing -setup -setup_margin 0.5 -repair_tns 100 -max_buffer_percent 50} m]} { puts "REPAIR_TIMING_WARN $m" }
puts "AFTER_REPAIR_SPEF_PARTIAL"
report_wns -max
report_tns -max
...
puts "ELEC_AFTER"
if {[catch {puts "SLEW [sta::max_slew_violation_count] CAP [sta::max_capacitance_violation_count]"}]} {
  puts "ELEC_COUNT_FAIL"
}
if {[catch {unset_case_analysis [get_ports rst_n]}]} {}
if {[catch {puts "RESET_VISIBLE_SLEW [sta::max_slew_violation_count] CAP [sta::max_capacitance_violation_count]"}]} {
  puts "RESET_ELEC_COUNT_FAIL"
}
...
write_db $outdir/butterfold_top_co6a36_setup2.odb
write_def $outdir/butterfold_top_co6a36_setup2.def
puts "CO6A36_SETUP2_DONE"
exit
