# ECO8: remaining extracted-path _1/_2 class (nor3_1/oai21_1/mux2_1).
# No extra prefix buffers. Full non-PG rip.
...
set nswap 0
set fh [open $eco/eco8_swaps.txt r]
...
global_route -congestion_iterations 50 -verbose -guide_file $eco/eco8.guide
detailed_route -droute_end_iter 64 -or_seed 42 -verbose 1 -output_drc $eco/eco8.drc
write_db $eco/butterfold_top_eco8_routed.odb
write_def $eco/butterfold_top_eco8_routed.def

set rcx_max $pdk/libs.tech/librelane/rules.openrcx.gf180mcuD.max
if {[catch {
  extract_parasitics -ext_model_file $rcx_max
  write_spef $eco/eco8.max.spef
  read_spef $eco/eco8.max.spef
} emsg]} { puts "EXTRACT_WARN $emsg" }
puts "ECO8_EXTRACTED_WNS"
report_wns -max
report_tns -max
catch {report_wns -max > $out/eco8_spef_wns.rpt}
catch {report_tns -max > $out/eco8_spef_tns.rpt}
catch {report_checks -path_delay max -slack_max 0 -group_path_count 40 > $out/eco8_spef_violations.rpt}
catch {report_check_types -max_slew -max_cap -max_fanout -violators > $out/eco8_spef_electrical.rpt}
puts "ECO8_COMPLETE"
exit
