# CO.6a + electrical: aoi221_2 MX -> R180 at same origin on pgfix ODB.
# R180 is rail-legal on MX rows, same 11.76x5.04 bbox, CO.6a-clean, full _2 drive.
...
if {[catch {global_route -congestion_iterations 30 -verbose -guide_file $eco/co6a28.guide} gmsg]} { puts "GRT_WARN $gmsg" }
if {[catch {detailed_route -droute_end_iter 32 -or_seed 42 -verbose 1 -output_drc $eco/co6a28.drc} dmsg]} {
    puts "DRT_WARN $dmsg"
} else { puts "DRT_OK" }
...
write_spef $cand/co6a28.max.spef
...
puts [format "DIE %.1f %.1f" [expr {[$die xMax]*1.0/$dbu}] [expr {[$die yMax]*1.0/$dbu}]]
puts "BTERMS [llength [$block getBTerms]]"
set nsram 0
foreach inst [$block getInsts] {
    if {[string match *sram256x8m8wm1* [[$inst getMaster] getName]]} { incr nsram }
}
puts "SRAM $nsram"

write_db $cand/butterfold_top_co6a28.odb
write_def $cand/butterfold_top_co6a28.def
write_verilog -include_pwr_gnd $cand/butterfold_top.co6a28.pnl.v
puts "CO6A28_DONE"
exit
