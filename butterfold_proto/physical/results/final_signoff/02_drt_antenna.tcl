# Native DRT + post-DRT antenna repair loop (LibreLane openroad/drt.tcl).
set proto /headless/aravindustries-repos/butterfold/butterfold_proto
set out $proto/physical/results/final_signoff
file mkdir $out/drt
file mkdir $out/antenna

read_db $out/butterfold_top_grt_antenna.odb
set_thread_count 22
...
detailed_route -droute_end_iter 64 -or_seed 42 -verbose 1 \
  -output_drc $out/drt/drt-run-${i}.drc
puts "DRT_RUNTIME_MS_$i [expr {[clock milliseconds]-$t0}]"
write_db $out/drt/drt-run-${i}.odb
incr i

# LibreLane: while {i <= DRT_ANTENNA_REPAIR_ITERS && [check_antennas]} { repair; drt }
while {$i <= $max_ant_iters} {
  puts "CHECK_ANTENNAS_BEFORE_REPAIR_ITER $i DIODE [diode_count]"
  set ant [check_antennas]
  puts "CHECK_ANTENNAS_RC $ant"
  if {!$ant} {
    puts "ANTENNA_PASS_AFTER_DRT_ITER [expr {$i-1}]"
    break
  }
  puts "ANTENNA_REPAIR_ITER $i"
  set d0 [diode_count]
  ...
  if {!$inserted && ($d1 == $d0)} {
    puts "NO_DIODES_INSERTED_ENDING_LOOP"
    break
  }
...
puts "FINAL_ANTENNA_CHECK DIODE [diode_count]"
check_antennas -verbose -report_file $out/antenna/final_verbose.rpt
