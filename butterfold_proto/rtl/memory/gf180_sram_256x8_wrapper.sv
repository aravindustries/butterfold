...
//
// Power-pin handling: the physical implementation flow connects the macro's
// VDD/VSS through the PDN (PDN_MACRO_CONNECTIONS), not through the netlist,
// and the LibreLane blackbox stub therefore declares no power ports.  The
// foundry behavioral model used for simulation does declare them, so those
// connections live behind GF180_SRAM_POWER_PINS -- defined for iverilog,
// left undefined for synthesis/PnR.
...
`ifdef SYNTHESIS
    ...
    (* keep *) gf180mcu_fd_sc_mcu7t5v0__nand2_4 u_cen_driver (
        .A1(rst_n),
        .A2(req_i),
        .ZN(macro_cen)
    );
    (* keep *) gf180mcu_fd_sc_mcu7t5v0__clkinv_4 u_gwen_driver (
        .I (write_i),
        .ZN(macro_gwen)
    );
    ...
    generate
        for (synth_pin = 0; synth_pin < 8; synth_pin = synth_pin + 1) begin : g_pin_drive
            (* keep *) gf180mcu_fd_sc_mcu7t5v0__buf_4 u_addr_driver (
                .I(addr_i[synth_pin]),
                .Z(macro_addr[synth_pin])
            );
            (* keep *) gf180mcu_fd_sc_mcu7t5v0__buf_4 u_data_driver (
                .I(wdata_i[synth_pin]),
                .Z(macro_wdata[synth_pin])
            );
        end
    endgenerate
`else
