...
module scheduler_realtime_full_tb;
    ...
    localparam integer RX_SHORT_SAMPLES_PER_TEST   = SHORT_NORMAL_CP + 128;
    localparam integer RX_LONG_SAMPLES_PER_TEST = LONG_NORMAL_CP + 128;
    localparam integer TX_SHORT_OUTPUT_SAMPLES     = SHORT_NORMAL_CP + 128;
    localparam integer TX_LONG_OUTPUT_SAMPLES   = LONG_NORMAL_CP + 128;
    ...
    logic [15:0] rx_short_inputs
        [0:NUM_OFDM_RX_TESTS*RX_SHORT_SAMPLES_PER_TEST-1];
    logic [15:0] rx_long_inputs
        [0:NUM_OFDM_RX_TESTS*RX_LONG_SAMPLES_PER_TEST-1];
    logic [15:0] rx_short_expected [0:NUM_OFDM_RX_TESTS*12-1];
    logic [15:0] rx_long_expected [0:NUM_OFDM_RX_TESTS*12-1];

    logic [15:0] tx_short_inputs [0:NUM_OFDM_TX_TESTS*12-1];
    logic [15:0] tx_long_inputs [0:NUM_OFDM_TX_TESTS*12-1];
    logic [15:0] tx_short_expected
        [0:NUM_OFDM_TX_TESTS*TX_SHORT_OUTPUT_SAMPLES-1];
    logic [15:0] tx_long_expected
        [0:NUM_OFDM_TX_TESTS*TX_LONG_OUTPUT_SAMPLES-1];
    logic [7:0] tx_received_bytes [0:2*TX_LONG_OUTPUT_SAMPLES-1];
    ...
    task automatic rx_send_ofdm_frame (
        ...
        begin
            ...
            if (long_cp_mode) begin
                base_index = test_index * RX_LONG_SAMPLES_PER_TEST;
                for (sample_index = 0;
                     sample_index < RX_LONG_SAMPLES_PER_TEST;
                     sample_index = sample_index + 1) begin
                    sample_word = rx_long_inputs[base_index + sample_index];
                    rx_send_byte(sample_word[15:8], unused_cycle);
                    rx_send_byte(sample_word[7:0], final_input_cycle);
                end
            end else begin
                base_index = test_index * RX_SHORT_SAMPLES_PER_TEST;
                for (sample_index = 0;
                     sample_index < RX_SHORT_SAMPLES_PER_TEST;
                     sample_index = sample_index + 1) begin
                    sample_word = rx_short_inputs[base_index + sample_index];
                    rx_send_byte(sample_word[15:8], unused_cycle);
                    rx_send_byte(sample_word[7:0], final_input_cycle);
                end
            end
    ...
    task automatic rx_receive_and_check_frame (
        ...
        begin
            ...
            expected_base = test_index * 12;

            while (byte_index < 24) begin
                @(posedge clk);
                ...
                if (dout_valid_o) begin
                    ...
                    expected_word = long_cp_mode
                        ? rx_long_expected[expected_base + bin_index]
                        : rx_short_expected[expected_base + bin_index];
                    ...
                    wait_cycles = wait_cycles + 1;
                    if (started)
                        $fatal(1, "dout_valid_o dropped inside the 24-byte one-RB OFDM_RX burst");
                    if (wait_cycles >= MAX_WAIT_CYCLES)
        ...
        begin
            ...
            if (frame_passed) begin
                ...
                $display(
                    ...
                    long_cp_mode ? "LONG_NORMAL_CP" : "SHORT_NORMAL_CP",
                    test_index,
                    command_to_first,
                    input_to_first,
                    command_to_last
    ...
    task automatic tx_send_tx_frame (
        ...
        begin
            ...
            for (sample_index = 0; sample_index < 12;
                 sample_index = sample_index + 1) begin
                sample_word = long_cp_mode
                    ? tx_long_inputs[base_index + sample_index]
                    : tx_short_inputs[base_index + sample_index];
    ...
    task automatic tx_receive_and_check_frame (
        ...
        logic [7:0] expected_byte;
        begin
            cp_length = long_cp_mode ? LONG_NORMAL_CP : SHORT_NORMAL_CP;
            ...
            while (byte_index < output_bytes) begin
                ...
                if (dout_valid_o) begin
                    ...
                    expected_word = long_cp_mode
                        ? tx_long_expected[expected_base + sample_index]
                        : tx_short_expected[expected_base + sample_index];
                    expected_byte = byte_index[0]
                        ? expected_word[7:0]
                        : expected_word[15:8];
    ...
    task automatic run_ofdm_tx_mode (
        ...
        begin
            ...
            output_bytes = 2 * (128 + (long_cp_mode ? LONG_NORMAL_CP : SHORT_NORMAL_CP));
            ...
            if (frame_passed) begin
                ...
                $display(
                    ...
                    long_cp_mode ? "long normal CP" : "short normal CP",
                    test_index,
                    first_output_cycle - command_cycle,
                    final_output_cycle - command_cycle
                );
    ...
    initial begin
        ...
        rx_failed_frames = 0;
        rx_matched_bytes = 0;
        rx_total_bytes = 2 * NUM_OFDM_RX_TESTS * 24;
        rx_abs_error_sum = 0;
        rx_max_abs_error = 0;
        ...
        $display("============================================================");
        $display("6/7 OFDM_RX short normal CP and long normal CP regression");
        $display("============================================================");
        ...
        $display("============================================================");
        $display("7/7 DFT-s-OFDM TX short normal CP and long normal CP regression");
        $display("============================================================");
    ...
    initial begin
        $dumpfile("scheduler_realtime_full_tb.vcd");
        $dumpvars(0, scheduler_realtime_full_tb);
    end
