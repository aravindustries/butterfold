...
module scheduler #(
    ...
    // OFDM_RX:    command, CP samples, then 128 useful TDIQ samples.
    // DFTS_TX:    command, then 12 frequency-domain complex samples.
    input  logic [7:0] din,
    ...
    localparam logic [3:0] OFDM_NORMAL_CP_LENGTH   = 4'd9;
    localparam logic [3:0] OFDM_EXTENDED_CP_LENGTH = 4'd10;
    ...
    assign ofdm_capture_start =
        ...
        ((din == CMD_OFDM_RX_NORMAL_CP) ||
         (din == CMD_OFDM_RX_EXTENDED_CP));

    assign ofdm_capture_cp_length =
        (din == CMD_OFDM_RX_EXTENDED_CP)
            ? OFDM_EXTENDED_CP_LENGTH
            : OFDM_NORMAL_CP_LENGTH;
    ...
    assign tx_capture_start =
        (rx_state == RX_COMMAND) && din_fire &&
        ((din == CMD_OFDM_TX_NORMAL_CP) ||
         (din == CMD_OFDM_TX_EXTENDED_CP));
    ...
    assign bf_out_ready =
        fft128_active
            ? ((fft128_state == F128_WAIT_RESULT)
                ? (fft128_final_stage
                    ? (fft128_ofdm_active ? 1'b1 : result_ready_i)
                    : 1'b1)
                : 1'b0)
            : (dft12_active
                ? ((dft12_state == D12_WAIT_RESULT)
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            tx_cp_length_reg      <= OFDM_NORMAL_CP_LENGTH;
            ...
            if (rx_state == RX_COMMAND && din_fire) begin
                ...
                if ((din == CMD_OFDM_RX_NORMAL_CP) ||
                    (din == CMD_OFDM_RX_EXTENDED_CP)) begin
                    ofdm_fft_block_ready <= 1'b0;
                end

                if ((din == CMD_OFDM_TX_NORMAL_CP) ||
                    (din == CMD_OFDM_TX_EXTENDED_CP)) begin
    ...
    always @(posedge clk) begin
        if (ofdm_input_write) begin
            fft_ram_i[bit_reverse7(ofdm_sample_index)] <= ofdm_sample_i;
            fft_ram_q[bit_reverse7(ofdm_sample_index)] <= ofdm_sample_q;
        end else if (tx_mapper_write_valid) begin
            fft_ram_i[tx_mapper_write_addr] <= tx_mapper_write_i;
            fft_ram_q[tx_mapper_write_addr] <= tx_mapper_write_q;
            ...
            fft_ram_q[bit_reverse7(rx_fft128_index)] <=
                sign_extend_q17(din);
        end else if (tx_input_write) begin
    ...
    fdiq_output_adapter u_fdiq_output_adapter (
        .clk           (clk),
        .rst_n         (rst_n),
        .start_i       (ofdm_output_start),
        .sample_addr_o (ofdm_output_addr),
        .sample_i_i    (fft_ram_i[ofdm_output_addr]),
        .sample_q_i    (fft_ram_q[ofdm_output_addr]),
        .dout           (ofdm_rx_dout),
    ...
    tdiq_output_cp_insert u_tdiq_output_cp_insert (
        .clk           (clk),
        .rst_n         (rst_n),
        .start_i       (tx_output_start),
        .cp_length_i   (tx_cp_length_reg),
        .sample_addr_o (tx_output_addr),
        .sample_i_i    (fft_ram_i[tx_output_addr]),
        .sample_q_i    (fft_ram_q[tx_output_addr]),
        .dout           (tx_dout),
