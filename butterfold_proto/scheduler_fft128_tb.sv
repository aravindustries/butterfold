...
module scheduler_fft128_tb;
    ...
    task automatic send_fft128_request (
            ...
            while (!accepted) begin
                ...
                if (din_valid_i && din_ready_o) begin
                    ...
                    if (wait_cycles >= MAX_WAIT_CYCLES)
                        $fatal(1, "Timeout sending FFT128 command");
                end
            end
            ...
            for (sample_index = 0; sample_index < N; sample_index = sample_index + 1) begin
                ...
                while (!accepted) begin
                    ...
                    if (din_valid_i && din_ready_o) begin
                        ...
                        wait_cycles = wait_cycles + 1;
                        if (wait_cycles >= MAX_WAIT_CYCLES)
                            $fatal(1, "Timeout sending FFT128 sample %0d I", sample_index);
                    end
                end
                ...
                while (!accepted) begin
                    ...
                    if (din_valid_i && din_ready_o) begin
                        ...
                        wait_cycles = wait_cycles + 1;
                        if (wait_cycles >= MAX_WAIT_CYCLES)
                            $fatal(1, "Timeout sending FFT128 sample %0d Q", sample_index);
                    end
                end
            end
    ...
    task automatic receive_fft128_results (
        ...
        begin
            ...
            while (transaction_count < N/2) begin
                ...
                if (result_valid_o && result_ready_i) begin
                    ...
                    wait_cycles = wait_cycles + 1;
                    if (wait_cycles >= MAX_WAIT_CYCLES)
                        $fatal(1, "Timeout waiting for FFT128 result transaction %0d",
                               transaction_count);
                end
            end

            if (last_result_cycle < 0)
                $fatal(1, "FFT128 completed without result_last_o");

            for (bin_index = 0; bin_index < N; bin_index = bin_index + 1) begin
                if (!bin_seen[bin_index])
                    $fatal(1, "FFT128 output bin %0d was never received", bin_index);
            end
        end
    endtask
    ...
    initial begin
        ...
        $readmemh("vectors/fft128_inputs.hex", input_vectors);
        $readmemh("vectors/fft128_expected.hex", expected_vectors);

        reset_dut();

        $display("");
        $display("============================================================");
        $display("FFT128 scheduler verification");
        ...
        $display("");
        $display("============================================================");
        $display("FFT128 verification summary");
        $display("============================================================");
    ...
    initial begin
        $dumpfile("scheduler_fft128_tb.vcd");
        $dumpvars(0, scheduler_fft128_tb);
    end
