...
module scheduler #(
    ...
    // Byte-stream FDIQ output used by OFDM_RX modes. The receiver is assumed
    // always ready whenever dout_valid_o is asserted.
    ...
    assign dft12_start =
        dft12_block_ready && !fft128_block_ready && !ofdm_fft_block_ready &&
    ...
    assign bf_out_ready =
        fft128_active
            ...
            : (dft12_active
                ? ((dft12_state == D12_WAIT_RESULT)
                    ? (dft12_final_operation ? result_ready_i : 1'b1)
                    : 1'b0)
    ...
    assign ofdm_output_start = fft128_done && fft128_ofdm_active;
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (fft128_done && fft128_ofdm_active)
                ofdm_fft_block_ready <= 1'b0;
    ...
    always @(posedge clk) begin
        if (ofdm_input_write) begin
            ...
            case (dft12_phase)
                ...
                default: begin
                    // Final FFT4 outputs are sent directly downstream.
                end
            endcase
        end
    end
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (fft128_start) begin
                fft128_active         <= 1'b1;
                fft128_ofdm_active    <= ofdm_fft_block_ready;
                fft128_inverse_active <= ofdm_fft_block_ready
                    ? 1'b0
