...
module scheduler #(
    ...
    // Result metadata can cover every queued FFT2/IFFT2 plus the butterfly's
    ...
    // One metadata entry is pushed whenever a two-point operation is fully
    // issued to the butterfly. Results return in issue order, so the head bit
    // tells the output router whether the current result needs IFFT2 /2
    // normalization.
    ...
    assign result_meta_pop   = fft2_result_fire;
    assign fft2_result_inverse =
        !result_meta_empty &&
    ...
    // IFFT128 divides by 128 after the final stage. IFFT2 divides its
    // two-point sum/difference by two. FFT2 and FFT128 remain unscaled.
    assign routed_X0_i =
        (fft128_active && fft128_inverse_active && fft128_final_stage)
            ? ($signed(bf_X0_i) >>> 7)
            : ((!fft128_active && fft2_result_inverse)
                ? ($signed(bf_X0_i) >>> 1)
                : bf_X0_i);
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (
                rx_state == RX_COMMAND &&
                din_fire &&
                ((din == CMD_FFT2) || (din == CMD_IFFT2))
            ) begin
                rx_fft2_inverse <= (din == CMD_IFFT2);
            end
            ...
            if (fifo_push) begin
                ...
                fifo_inverse[fifo_write_pointer] <= rx_fft2_inverse;
                fifo_write_pointer <=
            ...
            case ({result_meta_push, result_meta_pop})
                2'b10:
                    result_meta_count <= result_meta_count + 1'b1;

                2'b01:
                    result_meta_count <= result_meta_count - 1'b1;

                default:
                    result_meta_count <= result_meta_count;
            endcase
